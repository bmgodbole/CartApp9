...
import com.bg.service.impl.FixedPriceOffer;
...
public class DiscountItemDao {
   ...
   public String getOfferDesc(int pId, String pdisCat)
   {
	   String offerDesc = null;
		query = "Select disc_desc from item_discount where prod_d_id = ? and discount_category = ?";
		try {
			pst = this.con.prepareStatement(query);
		    pst.setInt(1, pId);
		    pst.setString(2, pdisCat);
		    rs=pst.executeQuery();
		    while(rs.next())
			{
		    	offerDesc = rs.getString("disc_desc").toString();
   ...
   public ArrayList<AbsOffer> getOffers(int pId)
   {
	...
	try {
		...
		//	offerTypes.add(oft);
		   ...
		   break;
		   case "FIXED" :
				itemOffer = new FixedPriceOffer(rs.getInt("ATTR1"));
				itemOffers.add(itemOffer);
			   break;
		   default:
		   break;
